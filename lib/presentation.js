...
var CommentsAuthorsFragment = require('./fragments/js/commentsAuthorsFragment');
...
Presentation.relationshipTypes = {
...
Presentation.prototype.getSlideCount = function() {
	return this.getSlideKeys().length;
};
...
Presentation.prototype.addSlide = function(layoutName) {
	...
	delete slideContent["p:sld"]['$']["preserve"];
    delete slideContent["p:sld"]['$']["type"];
...
/**
 * @desc returns an array of existing comment authors
 * @returns {array} aCommentAuthors
 */
Presentation.prototype.getCommentAuthors = function() {
	//create authors xml
	var sCommentAuthorsPath = "ppt/commentAuthors.xml";
	if(!this.content[sCommentAuthorsPath]) {
		this.content[sCommentAuthorsPath] = clone(CommentsAuthorsFragment);
		//register globally
		this.content["[Content_Types].xml"]["Types"]["Override"]
			.push({
				"$" : {
					"PartName" : "/" + sCommentAuthorsPath,
					"ContentType" : "application/vnd.openxmlformats-officedocument.presentationml.commentAuthors+xml"
				}
		});
		this.addRel("COMMENT_AUTHORS", { target: "commentAuthors.xml" });
	}
	return this.content[sCommentAuthorsPath]["p:cmAuthorLst"]["p:cmAuthor"];
};

/**
 * @param {object} oAuthor
 * @param {string} oAuthor.name
 * @param {string} oAuthor.initials
 * @returns {string} authorId
 */
Presentation.prototype.addCommentAuthor = function(oAuthor) {
	var aAuthors = this.getCommentAuthors();
	if(!aAuthors) throw new Error("Failed to add new author. No authors xml existing.");
	var sAuthorId = aAuthors.length;
	aAuthors && aAuthors.push({
		"$": {
			"id": sAuthorId,
			"name": oAuthor.name || "",
			"initials": oAuthor.initials || oAuthor.name.substr(0,2),
			"clrIdx": sAuthorId
		}
	});
	return sAuthorId;
...
Presentation.prototype.getSlideKeys = function(){
	return Object.keys(this.content).filter(sKey => sKey.substr(0, 16) === "ppt/slides/slide");
};

/**
 * @param {string} sSlideName
 */
Presentation.prototype.getSlideIndex  = function(sSlideName) {
	var iSlideIndex;
	this.getSlideKeys().forEach((sKey, iIndex) => { if(sKey.split("/")[2].split(".")[0] === sSlideName) iSlideIndex = iIndex; });
	return iSlideIndex + 1;
};

/**
 * @desc add relationship node to the presentation relationship definition
 * @param {string} type
 ...
 * 
 */
Presentation.prototype.addRel = function (type, options) {
	...
	var oRelation = {
		"$" : {
			"Id" : rId,
			"Type" : Presentation.relationshipTypes[type],
...
Presentation.prototype.getRelations = function() {
	return this.content['ppt/_rels/presentation.xml.rels']["Relationships"]["Relationship"];
};

Presentation.prototype.getAuthorById = function(iId) {
	return this.getCommentAuthors().find(oAuthor => oAuthor["$"]["id"] == iId);
};
