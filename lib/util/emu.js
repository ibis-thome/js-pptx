module.exports = {
	...
	calculateEMU : function(position) {
		...
		var multiplier = this.types[type] || 1;
		
		oEMUPosition.x = isNaN(oEMUPosition.x) ? 0 : Math.floor(Math.floor(oEMUPosition.x) * multiplier);
		oEMUPosition.y = isNaN(oEMUPosition.y) ? 0 : Math.floor(Math.floor(oEMUPosition.y) * multiplier);
		oEMUPosition.w = isNaN(oEMUPosition.w) ? 0 : Math.floor(Math.floor(oEMUPosition.w) * multiplier);
		oEMUPosition.h = isNaN(oEMUPosition.h) ? 0 : Math.floor(Math.floor(oEMUPosition.h) * multiplier);
